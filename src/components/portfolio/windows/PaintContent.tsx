...
import freeSelectIcon from "../../../assets/freeform_select.svg"
import fillIcon from "../../../assets/bucket.svg"
import pickIcon from "../../../assets/pick_color.svg"
import zoomIcon from "../../../assets/magnifier.svg"
import brushIcon from "../../../assets/brush.svg"
import airbrushIcon from "../../../assets/airbrush.svg"
...
const TOOL_GRID: { id: Tool; title: string }[][] = [
	[
		{ id: "free-select", title: "Free Select" },
		{ id: "rect-select", title: "Selection" },
	],
	[
	...
	[
		...
		{ id: "rounded-rect", title: "Rounded Rect." },
	],
]
...
function ToolIcon({ id }: { id: Tool }) {
	...
	const icon = (src: string) => <img src={src} width={16} height={16} draggable={false} alt="" />
	switch (id) {
		case "pencil":
			return icon(pencilIcon)
		case "eraser":
			return icon(eraserSvg)
		case "free-select":
			return icon(freeSelectIcon)
		case "fill":
			return icon(fillIcon)
		case "pick":
			return icon(pickIcon)
		case "zoom":
			return icon(zoomIcon)
		case "brush":
			return icon(brushIcon)
		case "airbrush":
			return icon(airbrushIcon)
		case "text":
			return icon(textIcon)
		case "curve":
			return icon(curveIcon)
		case "polygon":
			return icon(polygonIcon)
		case "rect-select":
			return (
				<svg style={s} viewBox="0 0 16 16">
					<rect x="2" y="3" width="11" height="9" fill="none" stroke="#000" strokeWidth="1" strokeDasharray="2,1" />
				</svg>
			)
		case "line":
			return (
				<svg style={s} viewBox="0 0 16 16">
					<line x1="2" y1="14" x2="14" y2="2" stroke="#000" strokeWidth="1.5" />
				</svg>
			)
		case "rect":
			return (
				<svg style={s} viewBox="0 0 16 16">
					<rect x="2" y="3" width="11" height="9" fill="none" stroke="#000" strokeWidth="1.5" />
				</svg>
			)
		case "ellipse":
			return (
				<svg style={s} viewBox="0 0 16 16">
					<ellipse cx="8" cy="8" rx="6" ry="4" fill="none" stroke="#000" strokeWidth="1.5" />
				</svg>
			)
		case "rounded-rect":
			return (
				<svg style={s} viewBox="0 0 16 16">
					<rect x="2" y="3" width="11" height="9" rx="3" ry="3" fill="none" stroke="#000" strokeWidth="1.5" />
				</svg>
			)
		default:
			return (
...
export function PaintContent() {
	...
	const onMouseMove = useCallback(
		(e: React.MouseEvent<HTMLCanvasElement>) => {
			...
			if (t === "polygon" && polygonPtsRef.current.length > 0) {
				drawPolyOverlay(polygonPtsRef.current, pos)
				return
			}
			...
			if (t === "curve" && curveDataRef.current) {
				...
				ov.quadraticCurveTo(pos.x, pos.y, end.x, end.y)
				ov.stroke()
				return
			}
			...
			} else if (t === "rounded-rect") {
				const w = pos.x - sx,
					h = pos.y - sy
				ov.beginPath()
				ov.roundRect(sx, sy, w, h, Math.min(Math.abs(w), Math.abs(h)) * 0.15)
				ov.stroke()
			} else if (t === "ellipse") {
				const rx = Math.abs(pos.x - sx) / 2,
					ry = Math.abs(pos.y - sy) / 2
				ov.beginPath()
				ov.ellipse(sx + (pos.x - sx) / 2, sy + (pos.y - sy) / 2, rx, ry, 0, 0, Math.PI * 2)
				ov.stroke()
			} else if (t === "curve") {
				ov.beginPath()
				ov.moveTo(sx, sy)
		...
		(e: React.MouseEvent<HTMLCanvasElement>) => {
			...
			if (t === "line") {
				...
				const w = pos.x - sx,
					h = pos.y - sy
				ctx.beginPath()
				ctx.roundRect(sx, sy, w, h, Math.min(Math.abs(w), Math.abs(h)) * 0.15)
				ctx.stroke()
			} else if (t === "ellipse") {
				const rx = Math.abs(pos.x - sx) / 2,
					ry = Math.abs(pos.y - sy) / 2
		...
		<div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: FONT, userSelect: "none" }}>
			...
			<div style={{ display: "flex", flex: 1, minHeight: 0, gap: 2, padding: 2 }}>
				...
				<div style={{ ...raised, flexShrink: 0, background: BG, padding: 2, display: "flex", flexDirection: "column", gap: 2, alignSelf: "flex-start" }}>
					...
					{zoom !== 1 && <div style={{ fontSize: 9, textAlign: "center", color: "#000080", paddingTop: 2 }}>×{zoom}</div>}
				</div>
				...
				<div style={{ ...sunken, flex: 1, overflow: "auto", background: "#808080" }}>
					<div style={{ position: "relative", display: "inline-block", margin: 4 }}>
						<canvas
							...
							onMouseLeave={(e) => {
								setCursorPos(null)
								if (drawingRef.current) onMouseUp(e)
							}}
							onContextMenu={(e) => {
								e.preventDefault()
								e.stopPropagation()
								if (toolRef.current === "zoom") setZoom((z) => Math.max(1, z / 2))
							}}
						/>
						<canvas
							ref={overlayRef}
						...
						{textPos && (
							<input
								...
								style={{
									position: "absolute",
									left: textPos.x * zoom,
									top: textPos.y * zoom - 16,
									background: "transparent",
									border: "1px dashed #000080",
									outline: "none",
									fontSize: (12 + size * 2) * zoom,
									fontFamily: FONT,
									color,
									minWidth: 60,
									padding: "0 2px",
				...
				<div style={{ display: "grid", gridTemplateColumns: "repeat(14, 16px)", gridTemplateRows: "repeat(2, 13px)", gap: 1 }}>
					{PALETTE.map((c, i) => (
						<div
							...
							style={{ ...raised, width: 14, height: 13, background: c, cursor: "default" }}
							title={c}
						/>
					))}
				</div>
			...
			<div style={{ display: "flex", background: BG, flexShrink: 0 }}>
				...
				<div style={{ ...sunken, width: 120, padding: "1px 6px", fontSize: 11 }}>{cursorPos && `x : ${cursorPos.x}`}</div>
				<div style={{ ...sunken, width: 80, padding: "1px 6px", fontSize: 11 }}>{cursorPos && `y : ${cursorPos.y}`}</div>
			</div>
		</div>
	)
}
